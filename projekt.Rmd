...
Naslednji graf prikazuje število presenečenj v vsaki sezoni posamezne lige. Za presenečenje se smatra, ko ekipa, ki je uvrščena več kot 5 mest nižje na lestvici premaga višje uvrščeno ekipo. V analizo sem vzel le tekme po 5. kolu, ko so ekipe že nekoliko razvščene po moči. Sprva je potrebno razumeti, da se v nemški ligi igra le 34 tekem na sezono, za razliko od 38 v ostalih ligah, tako je zaradi manjšega števila tekem tudi manj možnosti za presenečenje. Kljub temu lahko sklepamo, da v tej obstaja jasna razlika po moči ekip in imajo nižje uvščene ekipe manjšo možnost presenečenja, še posebej na gostovanju. Obratno lahko sklepamo za špansko ligo, kjer je v povprečju največ presenečenj. Vsem ligam pa je skupen upad števila presenečenj domače ekipe v sezoni 20/21, ko na stadionih ni bilo navijačev, ki bi lahko navdihnili ekipo. Odsotnost navijačev pa se je poznala tudi za močnejše ekipe, saj se je skoraj v vseh ligah bistveno povečalo število presenečenj gostujoče ekipe.
...
Tabeli sem nato preimenoval stolpce v pojasnjevalne spremenljivke `xi` in odvisno spremenljivko `yn` ter dodal stolpec `yd` za razvščevalni model in odstranil stolpec `Ekipa`, tako, da so ostali le še stolpci številskih spremenljivk. Preizkusil sem 5 modelov tri z algoritmom naključnih gozdovov in dva z linearno regresijo. Napovedno napako modelov pa sem ocenil po metodi prečnega preverjanja.

Izkazalo se je, da je pri napovedovanju pričakovanega števila točk prišlo do zelo velike napake, ne glede na izbiro modela. Razlog v tem je, da so vrednosti `yn` lahko le `0,1,3` tako je pri napačni napovedi bila napaka vedno velikosti vsaj `1`.

Tako sem se odločil narediti še razvrščevalni model, ki bi napovedoval ali bo ekipa na tekmi zmagala ali ne. Če je bilo število osvojenih točk na tekmi enako 3, je bila vrednost `yd = 1`, sicer je bila `-1`. Vzel sem enakih 5 modelov in jih spet  razdelil na tri z algoritmom naključnih gozdov in dva z logistično regresijo. Na spodnjem grafu so prikazane napovedne napake modelov.

```{r graf_napak, echo=FALSE, fig.align='center'}
graf_napak
```

Kot najboljši se je izkazal model z algoritmom logistične regresije `yd ~ x1 + x2 + x3 + x4 + x5 + x6`, v katerega so vključene vse pojasnjevalne spremenljivke. Napake so tudi v razvrščevalnem modelu razmeroma viskoke, kar nakazuje na to, da v modelu nimamo dostopa do pomembnih podatkov, ki vplivajo na rezultat, npr. poškodbe pomembnih igralcev v ekipi.
